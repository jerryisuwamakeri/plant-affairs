    ...
    <>
            ...
            {filteredServices.map((s) => (
              <div
                ...
                className={`flex flex-col border p-6 ${
                  s.featured ? "bg-forest-800 text-white border-forest-800" : "bg-white border-warm-200"
                }`}
              >
                {s.featured && (
                ...
                <h3 className={`font-serif text-xl mb-2 ${s.featured ? "text-white" : "text-forest-800"}`}>{s.name}</h3>
                <p className={`text-sm leading-relaxed mb-4 ${s.featured ? "text-white/70" : "text-stone-500"}`}>{s.description}</p>
                {s.duration && (
                  <p className={`text-xs mb-2 ${s.featured ? "text-sage" : "text-stone-400"}`}>Timeline: {s.duration}</p>
                )}
                <div className={`font-serif text-3xl mb-1 ${s.featured ? "text-sage" : "text-forest-700"}`}>{formatPrice(s.price)}</div>
                {s.priceNote && (
                  <p className={`text-xs mb-5 ${s.featured ? "text-white/50" : "text-stone-400"}`}>{s.priceNote}</p>
                )}
      ...
      <section className="bg-forest-800 text-white py-14">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="font-serif text-3xl md:text-4xl font-light mb-4">Not sure where to start?</h2>
          <p className="text-white/60 text-sm mb-8">
            Our team of horticulturists is happy to advise on the right plants or service for your space. Call us or book a free 15-minute consultation.
          </p>
