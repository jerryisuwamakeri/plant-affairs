...
import type { GardenProduct, ServicePackage, CategoryMeta } from "@/lib/types";

interface Props {
  isGardenNeeds: boolean;
  isLandscaping: boolean;
  meta: CategoryMeta;
  allServices: ServicePackage[];
  allProducts: GardenProduct[];
}

function ServiceContentInner({ isGardenNeeds, isLandscaping, meta, allServices, allProducts }: Props) {
  ...
  const filteredServices =
    activeSub === "all" || activeSub === "services"
      ? allServices
      : allServices.filter((s) => s.subcategory === activeSub);
      ...
      {showProducts && isGardenNeeds && (
        <section className="max-w-7xl mx-auto px-4 py-12">
          <h2 className="font-serif text-2xl text-forest-800 mb-6">
            {activeSub === "all"
              ? "All Products"
              : meta.subcategories.find((s) => s.slug === activeSub)?.label}
          </h2>
              ...
              {filteredServices.map((s) => (
                <div
                  ...
                  className={`bg-white border p-6 ${
                    s.featured ? "border-forest-400 ring-1 ring-forest-200" : "border-warm-200"
                  }`}
                >
                  {s.featured && (
                ...
                <ul className="space-y-2.5 mb-6 flex-1">
                  {s.features.map((f) => (
                    <li key={f} className={`flex gap-2 text-xs ${s.featured ? "text-white/80" : "text-stone-600"}`}>
                      <Check
                        size={13}
                        className={s.featured ? "text-sage flex-shrink-0 mt-0.5" : "text-forest-500 flex-shrink-0 mt-0.5"}
                      />
                      {f}
                    </li>
                  ))}
